---
title: "Longitudinal Data with FrengressionSeq"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Longitudinal Data with FrengressionSeq}
...
`frengression_seq()` extends the base model to time-varying
(longitudinal) data. It maintains separate networks for each time step,
modeling the sequential dependencies: $S \to (X_0, Z_0) \to (X_1, Z_1) \to \cdots$.
...
```{r joint, eval = torch::torch_is_installed()}
...
```{r counterfactual, eval = torch::torch_is_installed()}
...
```{r plot, eval = torch::torch_is_installed(), fig.cap = "Counterfactual trajectories"}
